...
public:
    ...
    double alpham (double v){
        ...
        return fabs(theta)<0.0001? 1.0 : 0.75*1*theta/(1.0-exp(-theta));
    }
    
    double betam(double v){
        return 0.75*4.0*exp(-(v+70.0)/18.0);
    }
    
    double alphah(double v){
        return 1*0.07*exp(-(v+70.0)/20.0);
    }
    
    double betah(double v){
        return 1*1.0/(1.0+exp(-(v+40.0)/10.0));
    }
    
    double alphan(double v){
        double theta = (v+60.0)/10.0;
        return fabs(theta)<0.0001? 0.1 : 1*1*0.1*theta/(1.0 - exp(-theta));
    }
    
    double betan(double v){
        return 1* 0.125 * exp(-(v+70.0)/80.0);
    }
...
//         double var;
//         for (int i = 0; i < freq.size(); i++){
//             if (cos(2.0 * M_PI * freq[i] * t / 1000.0) > 0) {var = 1;}
//             else {var = 0;}
//           // var = cos(2.0 * M_PI * freq[i] * t / 1000.0);
//            var = pow(-1,var );
//            ix += amp[i] * var;
//         }
        ...
        if(  ((t>116.1)&&(t<150))||((t>200.1)&&(t<250))  )
        {ix = 1.00*ix;}
